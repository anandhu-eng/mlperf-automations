...
def preprocess(i):
    ...
    tp_size = env.get("MLC_VLLM_SERVER_TP_SIZE", False)
    if tp_size:
        cmd_args += f" --tensor-parallel-size {tp_size}"

    pp_size = env.get("MLC_VLLM_SERVER_PP_SIZE", False)
    if pp_size:
        cmd_args += f" --pipeline-parallel-size {pp_size}"

    api_key = env.get("MLC_VLLM_SERVER_API_KEY", "root")
    if pp_size:
        cmd_args += f" --api-key {api_key}"

    distributed_executor_backend = env.get(
        "MLC_VLLM_SERVER_DIST_EXEC_BACKEND", False)
    if distributed_executor_backend:
        cmd_args += f" --distributed-executor-backend {distributed_executor_backend}"

    host = env.get("MLC_VLLM_SERVER_HOST", False)
    if host:
        cmd_args += f" --host {host}"

    port = env.get("MLC_VLLM_SERVER_PORT", False)
    if port:
        cmd_args += f" --port {port}"

    uvicorn_log_level = env.get("MLC_VLLM_SERVER_UVICORN_LOG_LEVEL", False)
    if uvicorn_log_level:
        cmd_args += f" --uvicorn-log-level {uvicorn_log_level}"

    allow_credentials = env.get("MLC_VLLM_SERVER_ALLOW_CREDENTIALS", False)
    if allow_credentials:
        cmd_args += f" --allow-credentials"

    allowed_origins = env.get("MLC_VLLM_SERVER_ALLOWED_ORIGINS", False)
    if allowed_origins:
        cmd_args += f" --allowed-origins {allowed_origins}"

    allowed_methods = env.get("MLC_VLLM_SERVER_ALLOWED_METHODS", False)
    if allowed_methods:
        cmd_args += f" --allowed-methods {allowed_methods}"

    allowed_headers = env.get("MLC_VLLM_SERVER_ALLOWED_HEADERS", False)
    if allowed_headers:
        cmd_args += f" --allowed-headers {allowed_headers}"

    lora_modules = env.get("MLC_VLLM_SERVER_LORA_MODULES", False)
    if lora_modules:
        cmd_args += f" --lora-modules {lora_modules}"

    prompt_adapters = env.get("MLC_VLLM_SERVER_PROMPT_ADAPTERS", False)
    if prompt_adapters:
        cmd_args += f" --prompt-adapters {prompt_adapters}"

    chat_template = env.get("MLC_VLLM_SERVER_CHAT_TEMPLATE", False)
    if chat_template:
        cmd_args += f" --chat-template {chat_template}"

    response_role = env.get("MLC_VLLM_SERVER_RESPONSE_ROLE", False)
    if response_role:
        cmd_args += f" --response-role {response_role}"

    ssl_keyfile = env.get("MLC_VLLM_SERVER_SSL_KEYFILE", False)
    if ssl_keyfile:
        cmd_args += f" --ssl-keyfile {ssl_keyfile}"

    ssl_certfile = env.get("MLC_VLLM_SERVER_SSL_CERTFILE", False)
    if ssl_certfile:
        cmd_args += f" --ssl-certfile {ssl_certfile}"

    ssl_ca_certs = env.get("MLC_VLLM_SERVER_SSL_CA_CERTS", False)
    if ssl_ca_certs:
        cmd_args += f" --ssl-ca-certs {ssl_ca_certs}"

    ssl_cert_reqs = env.get("MLC_VLLM_SERVER_SSL_CERT_REQS", False)
    if ssl_cert_reqs:
        cmd_args += f" --ssl-cert-reqs {ssl_cert_reqs}"

    root_path = env.get("MLC_VLLM_SERVER_ROOT_PATH", False)
    if root_path:
        cmd_args += f" --root-path {root_path}"

    middleware = env.get("MLC_VLLM_SERVER_MIDDLEWARE", False)
    if middleware:
        cmd_args += f" --middleware {middleware}"

    tokenizer = env.get("MLC_VLLM_SERVER_TOKENIZER", False)
    if tokenizer:
        cmd_args += f" --tokenizer {tokenizer}"

    skip_tokenizer_init = env.get("MLC_VLLM_SERVER_SKIP_TOKENIZER_INIT", False)
    if skip_tokenizer_init:
        cmd_args += f" --skip-tokenizer-init"

    revision = env.get("MLC_VLLM_SERVER_REVISION", False)
    if revision:
        cmd_args += f" --revision {revision}"

    code_revision = env.get("MLC_VLLM_SERVER_CODE_REVISION", False)
    if code_revision:
        cmd_args += f" --code-revision {code_revision}"

    tokenizer_revision = env.get("MLC_VLLM_SERVER_TOKENIZER_REVISION", False)
    if tokenizer_revision:
        cmd_args += f" --tokenizer-revision {tokenizer_revision}"

    tokenizer_mode = env.get("MLC_VLLM_SERVER_TOKENIZER_MODE", False)
    if tokenizer_mode:
        cmd_args += f" --tokenizer-mode {tokenizer_mode}"

    trust_remote_code = env.get("MLC_VLLM_SERVER_TRUST_REMOTE_CODE", False)
    if trust_remote_code:
        cmd_args += f" --trust-remote-code"

    download_dir = env.get("MLC_VLLM_SERVER_DOWNLOAD_DIR", False)
    if download_dir:
        cmd_args += f" --download-dir {download_dir}"

    load_format = env.get("MLC_VLLM_SERVER_LOAD_FORMAT", False)
    if load_format:
        cmd_args += f" --load-format {load_format}"

    dtype = env.get("MLC_VLLM_SERVER_DTYPE", False)
    if dtype:
        cmd_args += f" --dtype {dtype}"

    kv_cache_dtype = env.get("MLC_VLLM_SERVER_KV_CACHE_DTYPE", False)
    if kv_cache_dtype:
        cmd_args += f" --kv-cache-dtype {kv_cache_dtype}"

    quantization_param_path = env.get(
        "MLC_VLLM_SERVER_QUANTIZATION_PARAM_PATH", False)
    if quantization_param_path:
        cmd_args += f" --quantization-param-path {quantization_param_path}"

    max_model_len = env.get("MLC_VLLM_SERVER_MAX_MODEL_LEN", False)
    if max_model_len:
        cmd_args += f" --max-model-len {max_model_len}"

    guided_decoding_backend = env.get(
        "MLC_VLLM_SERVER_GUIDED_DECODING_BACKEND", False)
    if guided_decoding_backend:
        cmd_args += f" --guided-decoding-backend {guided_decoding_backend}"

    worker_use_ray = env.get("MLC_VLLM_SERVER_WORKER_USE_RAY", False)
    if worker_use_ray:
        cmd_args += f" --worker-use-ray"

    max_parallel_loading_workers = env.get(
        "MLC_VLLM_SERVER_MAX_PARALLEL_LOADING_WORKERS", False)
    ...
    speculative_max_model_len = env.get(
        "MLC_VLLM_SERVER_SPECULATIVE_MAX_MODEL_LEN", False)
    ...
    speculative_disable_by_batch_size = env.get(
        "MLC_VLLM_SERVER_SPECULATIVE_DISABLE_BY_BATCH_SIZE", False)
    ...
    spec_decoding_acceptance_method = env.get(
        "MLC_VLLM_SERVER_SPEC_DECODING_ACCEPTANCE_METHOD", False)
    ...
    typical_acceptance_sampler_posterior_threshold = env.get(
        "MLC_VLLM_SERVER_TYPICAL_ACCEPTANCE_SAMPLER_POSTERIOR_THRESHOLD", False)
    ...
    typical_acceptance_sampler_posterior_alpha = env.get(
        "MLC_VLLM_SERVER_TYPICAL_ACCEPTANCE_SAMPLER_POSTERIOR_ALPHA", False)
    ...
    model_loader_extra_config = env.get(
        "MLC_VLLM_SERVER_MODEL_LOADER_EXTRA_CONFIG", False)
    if model_loader_extra_config:
        cmd_args += f" --model-loader-extra-config {model_loader_extra_config}"

    preemption_mode = env.get("MLC_VLLM_SERVER_PREEMPTION_MODE", False)
    if preemption_mode:
        cmd_args += f" --preemption-mode {preemption_mode}"

    served_model_name = env.get("MLC_VLLM_SERVER_SERVED_MODEL_NAME", False)
    if served_model_name:
        cmd_args += f" --served-model-name {served_model_name}"

    qlora_adapter_name_or_path = env.get(
        "MLC_VLLM_SERVER_QLORA_ADAPTER_NAME_OR_PATH", False)
    ...
    otlp_traces_endpoint = env.get(
        "MLC_VLLM_SERVER_OTLP_TRACES_ENDPOINT", False)
    if otlp_traces_endpoint:
        cmd_args += f" --otlp-traces-endpoint {otlp_traces_endpoint}"

    engine_use_ray = env.get("MLC_VLLM_SERVER_ENGINE_USE_RAY", False)
    if engine_use_ray:
        cmd_args += f" --engine-use-ray"

    disable_log_requests = env.get(
        "MLC_VLLM_SERVER_DISABLE_LOG_REQUESTS", False)
    if disable_log_requests:
        cmd_args += f" --disable-log-requests"

    max_log_len = env.get("MLC_VLLM_SERVER_MAX_LOG_LEN", False)
    if max_log_len:
